...
def length_batching(
    ...
    """Set a maximum num_res cutoff to limit memory usage."""
...
def parse_chain_feats(chain_feats, scale_factor=1.0, center_pos=None, center_pos_noise=False, center_pos_noise_std=1.0):
    """
    Modified to accommodate receptor-peptide ligand scenario.
    """
    ...
    if center_pos is None:
        if np.sum(chain_feats['ligand_mask']) == 0:
            bb_center = np.zeros(3)
        else:
            peptide_pos = bb_pos[chain_feats['ligand_mask'] == 1]
            bb_center = np.sum(peptide_pos, axis=0) / (np.sum(chain_feats['ligand_mask']) + 1e-5)
    else:
        bb_center = center_pos

    # Add Gaussian noise to the center position. bb_center: x_1 in the illustration.
    if center_pos_noise:
        bb_center += np.random.normal(0, center_pos_noise_std, bb_center.shape)
    ...
    # Coordinate bias between the raw PDB file and the generated structure. pdb_center: x_0 in the illustration.
    pdb_center = chain_feats.get('center_pos', np.zeros(3))
    coordinate_bias = pdb_center + bb_center
    chain_feats['coordinate_bias'] = np.tile(coordinate_bias, (bb_pos.shape[0], 1)) # [N_res, 3]
...
def calc_distogram(pos, min_bin, max_bin, num_bins):
    dists_2d = torch.linalg.norm(
        pos[:, :, None, :] - pos[:, None, :, :], axis=-1)[..., None]  # [batch_size, N_res, N_res, 1]
    ...
    dgram = ((dists_2d > lower) * (dists_2d < upper)).type(pos.dtype)  # [batch_size, N_res, N_res, N_bins]
    return dgram
